...
const [selectedDoctor, setSelectedDoctor] = useState<DoctorProfileResponse | null>(null);  // ← add this
...
//change by nimesh - give access patient to consultation fee 
 function handleDoctorChange(event: React.ChangeEvent<HTMLSelectElement>) {
  ...
  setSuccessMessage("");
  setSelectedDoctor(doctor);   // ← add this
  setFormData((current) => ({
    ...current,
...
//done by nimesh
//1.validate consult fee exisits
//2.paymentStatus  "Pending" is hardcoded
//3.passing all the data to payment checkout page via location.state
 ...
 async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
  ...
  if (!selectedDoctor?.consultationFee) {
    setErrorMessage("Selected doctor has no consultation fee set. Please choose a different doctor.");
    return;
  }
  ...
  try {
    ...
    // Navigate to payment checkout with appointment + fee details
          ...
          <div className="p-6 sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                >
                  {isSubmitting ? "Creating Appointment..." : "Proceed to Payment"}
                </button>
